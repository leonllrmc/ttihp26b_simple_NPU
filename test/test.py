...
@cocotb.test()
async def npu_test_only_biais(dut):
    ...
    dummy_program_words = []
    for i in range(1, 9):
        dummy_program_words.append(to_instruction(is_end=0, act_function=0b000, bais=i))
        dummy_program_words.append(0x00)
        dummy_program_words.append(0x00)
        dummy_program_words.append(0x00)
        dummy_program_words.append(0x00)

    dummy_program_words.append(to_instruction(is_end=1, act_function=0b000, bais=i))
    ...
    while 1:
        ...
        if dut.user_project.simpleNPUTop.debug_CALC_COMPUTEMULACC_pulse.value:
            current_cycle += 1
            if True:
                dut._log.info(f"{dut.user_project.simpleNPUTop.instruction.value}")
...
@cocotb.test()
async def npu_test_weight_range(dut):
    # 1. Start the CPU clock (e.g., 50 MHz)
    clock = Clock(dut.clk, 20, unit="ns")
    cocotb.start_soon(clock.start())
    ...
    dummy_program_words = []
    for i in range(1, 9):
        dummy_program_words.append(to_instruction(is_end=0, act_function=0b000, bais=0))
        dummy_program_words.append(0x01)
        dummy_program_words.append(0x23)
        dummy_program_words.append(0x45)
        dummy_program_words.append(0x67)
...
@cocotb.test()
async def npu_test_weight_range2(dut):
    # 1. Start the CPU clock (e.g., 50 MHz)
    clock = Clock(dut.clk, 20, unit="ns")
    cocotb.start_soon(clock.start())
    ...
    for i in range(1, 9):
        dummy_program_words.append(to_instruction(is_end=0, act_function=0b000, bais=0))
        dummy_program_words.append(0x77)
        dummy_program_words.append(0x77)
        dummy_program_words.append(0x77)
        dummy_program_words.append(0x77)
